...
#  UMBRAL_BLOQUEO: fracción del ancho que debe ocupar para activar STOP
# ══════════════════════════════════════════════
...
#  OBS_FRAMES:    frames para calibrar el fondo
#  OBS_UMBRAL_DIFF: diferencia mínima en cualquier canal BGR
#                   para considerar un pixel como obstáculo (0-255)
#                   bajar = más sensible, subir = menos falsos positivos
#  OBS_AREA_MIN:  área mínima del contorno en px²
# ══════════════════════════════════════════════
OBS_FRAMES     = 60
OBS_UMBRAL_DIFF = 30
OBS_AREA_MIN   = 800
...
#  OBS_FRAMES_ACTIVAR: frames detectando para activar STOP
#  OBS_FRAMES_LIBERAR: frames sin detectar para liberar STOP
# ══════════════════════════════════════════════
...
estado = {
    ...
    "error_linea": None,
    "ultimo_tag": None,   # ID del último AprilTag detectado
}
estado_lock = threading.Lock()

# Fondo de referencia BGR
_fondo_ref   = None   # array float32 BGR promediado
_fondo_count = 0
...
def resetear_fondo():
    """Recalibra el fondo de referencia."""
...
def _mask_lineas(hsv):
    """Máscara de píxeles que son líneas roja o amarilla."""
...
def detectar_y_dibujar_obstaculos(frame, frame_deteccion=None):
    ...
    # Usar frame limpio para detección si se proporciona
    ...
    # ROI del frame limpio para detección
...
# ══════════════════════════════════════════════
def detectar_y_dibujar_qr(frame, detector_at):
    ...
    for tag in tags:
        ...
        y0 = int(min(pts[:, 1]))
        if info:
            label = f"TAG {tag_id} — {info['tipo'].upper()}"
        else:
            label = f"TAG {tag_id} — desconocido"
...
def _hilo_camara():
    ...
    while True:
        ...
        # Copiar frame limpio antes de dibujar para que la detección
        # de obstáculos no vea la interfaz gráfica de líneas
        ...
        frame          = detectar_y_dibujar_qr(frame, detector_at)
