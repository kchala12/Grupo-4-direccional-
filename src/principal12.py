...
OFFSET_CURVA     = 0      # offset en px al estar en curva viendo solo roja (ajustar)
VEL_BASE    = 50
VEL_MIN     = 25
VEL_MAX     = 75
INTERVALO_S = 0.1
...
#  PARÁMETROS DE CARGA
# ══════════════════════════════════════════════
DELAY_CARGA_S = 5   # segundos esperando que la carga suba al robot

# ══════════════════════════════════════════════
#  MAPA DE MISIONES
#  Asocia cada tag de misión con su secuencia
# ══════════════════════════════════════════════
MAPA_MISIONES = {
    2: SECUENCIA_MISION_1,
    # 3: SECUENCIA_MISION_2,  # agregar cuando esté definida
    # 4: SECUENCIA_MISION_3,  # agregar cuando esté definida
}
...
def _navegar_pid(segundos):
    ...
    while time.time() - t0 < segundos:
        ...
        if error is not None:
            ...
            offset   = OFFSET_CURVA if solo_roja else 0
...
# ══════════════════════════════════════════════
def _navegar_normal():
    """Ejecuta un ciclo de navegación normal. Retorna False si debe detenerse."""
    with _autonomo_lock:
        if not _autonomo:
            return False
    with camara_detector.estado_lock:
...
def _esperar_tag(tag_id):
    """Navega hasta detectar un tag específico. Retorna False si se detiene."""
    tag_ant = None
    while True:
        with _autonomo_lock:
            if not _autonomo:
...
def _hilo_autonomo(stop_ev):
    while not stop_ev.is_set():
        ...
        # ── FASE 1: navegar hasta estación de carga (tag 1) ──────────────
        print(f"{C.INFO}[AUTO] Navegando a estación de carga...{C.RESET}")
        if not _esperar_tag(1):
            continue

        # ── FASE 2: esperar carga ─────────────────────────────────────────
        print(f"{C.WARN}[CARGA] Esperando carga — {DELAY_CARGA_S}s{C.RESET}")
        motor_control.enviar("s 0")
        time.sleep(DELAY_CARGA_S)
        ...
        while secuencia is None:
            ...
            with camara_detector.estado_lock:
                tag_actual = camara_detector.estado["ultimo_tag"]
            if tag_actual in MAPA_MISIONES and tag_ant != tag_actual:
        ...
        # ── FASE 4: ejecutar secuencia de misión ──────────────────────────
        tag_visto_ant = tag_mision   # ya procesamos este tag
