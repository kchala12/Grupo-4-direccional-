...
# ✅ FIX: El firmware ESP32 en aplicar_comando() compara con strcmp(dir, "f"),
#    strcmp(dir, "b"), etc. — espera LETRAS, no palabras en español.
#    El mapa anterior ("f" → "adelante") causaba que todos los comandos
#    de movimiento fueran ignorados silenciosamente por la ESP32.
...
def enviar_cmd(cmd: str):
    ...
    Traduce la letra al formato que espera la ESP32.
    """
...
def set_velocidades(vel_izq: int, vel_der: int, dir_letra: str):
    ...
    """
    # ✅ FIX: el comando pid también usa letras: "pid 55 45 f"
    #    (no "pid 55 45 adelante" como hacía antes).
    dir_esp = _DIR_MAP.get(dir_letra, "f")
    pwm_cmd = f"pid {vel_izq} {vel_der} {dir_esp}"
    enviar_raw(pwm_cmd)
