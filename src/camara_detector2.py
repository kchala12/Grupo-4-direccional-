...
def _hilo_camara() -> None:
    ...
    while True:
        if picam is not None:
            ...
            # ✅ FIX: Picamera2 con formato RGB888 entrega RGB, pero OpenCV
            #    trabaja en BGR. Sin esta conversión los colores se invierten
            #    y la detección HSV de líneas falla completamente.
...
def iniciar(host="0.0.0.0", port=5000):
    ...
    # ✅ FIX: use_reloader=False es OBLIGATORIO cuando Flask corre en un hilo
    #    secundario. Sin esto Flask intenta relanzarse en el proceso principal
    #    (usando señales y fork), lo que bloquea stdin e impide que input()
    #    funcione en el hilo de la consola.
