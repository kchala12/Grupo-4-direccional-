...
def iniciar(host="0.0.0.0", port=5000):
    """
    Arranca el hilo de cámara y el servidor Flask en hilos daemon
    separados. No bloquea — la terminal queda libre para el principal.
    """
    ...
    def _flask():
        app.run(
            ...
            port=port,
            threaded=True,
            use_reloader=False,   # evita que Flask intente relanzar el proceso
            debug=False,          # evita que capture la terminal
        )
