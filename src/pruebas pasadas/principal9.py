...
# ══════════════════════════════════════════════
KP_DIR           = 0.08
KP_DIR_SOLO_ROJA = 0.15   # ganancia mayor cuando solo ve la roja
UMBRAL_CURVA     = 80     # error en px para considerar curva
OFFSET_CURVA     = 0      # offset en px al estar en curva (ajustar según pruebas)
VEL_BASE    = 50
VEL_MIN     = 25
VEL_MAX     = 75
INTERVALO_S = 0.1
...
def _navegar_pid(segundos):
    ...
    while time.time() - t0 < segundos:
        ...
        if error is not None:
            ...
            en_curva = abs(error) > UMBRAL_CURVA
            offset  = OFFSET_CURVA if (en_curva and solo_roja) else 0
