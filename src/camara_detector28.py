...
# ══════════════════════════════════════════════
OFFSET_CENTRO_DEFAULT = 50   # valor por defecto — ajustable desde principal
OFFSET_CENTRO         = OFFSET_CENTRO_DEFAULT
REF_SOLO_AMARILLA     = 0.25
REF_SOLO_ROJA_RECTA   = 0.95
REF_SOLO_ROJA_CURVA   = 0.50
UMBRAL_INCLINACION    = 90
...
def detectar_y_dibujar_lineas(frame):
    ...
    mask_a_izq = mask_a.copy()
    mask_a_izq[:, w // 2:] = 0
    cx_a, cy_a, cnt_a = _centroide(mask_a_izq, oy)
    ...
    fx_a = _x_promedio_franjas(mask_a_izq, franjas)
    ...
    def _fitline_roja():
        cnts_f, _ = cv2.findContours(mask_r, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
        if not cnts_f:
            return 0, 0, 1, 0, 0
        ...
        x0, y0 = int(line[2]), int(line[3]) + oy
        incl = abs(vx) / (abs(vy) + 0.001) * 100
        return incl, vx, vy, x0, y0

    incl_r, vx_r, vy_r, x0_r, y0_r = _fitline_roja()
    en_curva = (incl_r > UMBRAL_INCLINACION) and (fx_r is not None)

    if fx_r is not None:
        escala = 80
        p1 = (x0_r + int(vx_r * escala), y0_r + int(vy_r * escala))
        p2 = (x0_r - int(vx_r * escala), y0_r - int(vy_r * escala))
        cv2.arrowedLine(frame, p1, p2, C_LINEA_ROJA, 2, tipLength=0.3)

    error = None
    if fx_r is not None and fx_a is not None:
        # Usar OFFSET_CENTRO global (ajustable desde principal)
