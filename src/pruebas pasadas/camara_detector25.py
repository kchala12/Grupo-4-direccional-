...
REF_SOLO_ROJA_CURVA  = 0.70   # punto objetivo roja en curva
UMBRAL_INCLINACION   = 40     # diferencia en px entre franja sup e inf para detectar curva
...
def detectar_y_dibujar_lineas(frame):
    ...
    # Calcular inclinación de la roja con fitLine
    def _inclinacion_fitline(mask, offset_y=0):
        """Devuelve (inclinacion, vx, vy, x0, y0) usando fitLine sobre el contorno."""
        cnts_f, _ = cv2.findContours(mask, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
        if not cnts_f:
            return 0, 0, 1, 0, 0
        cnt_mayor = max(cnts_f, key=cv2.contourArea)
        if len(cnt_mayor) < 5:
            return 0, 0, 1, 0, 0
        line = cv2.fitLine(cnt_mayor, cv2.DIST_L2, 0, 0.01, 0.01)
        vx, vy, x0, y0 = float(line[0]), float(line[1]), float(line[2]), float(line[3])
        incl = abs(vx) / (abs(vy) + 0.001) * 100  # 0=vertical(recta), grande=inclinada(curva)
        return incl, vx, vy, int(x0), int(y0 + offset_y)

    incl_r, vx_r, vy_r, x0_r, y0_r = _inclinacion_fitline(mask_r, oy)
    en_curva = incl_r > UMBRAL_INCLINACION

    # Dibujar vector de la roja
    if fx_r is not None:
        escala = 80
        px1 = (x0_r - int(vx_r * escala), y0_r - int(vy_r * escala))
        px2 = (x0_r + int(vx_r * escala), y0_r + int(vy_r * escala))
        cv2.arrowedLine(frame, px1, px2, C_LINEA_ROJA, 2, tipLength=0.3)
    ...
    if fx_r is not None and fx_a is not None:
        ...
        cv2.putText(frame, f"{curva_txt} err:{error:+d}px {signo}", (10, 55),
        ...
        ref_x = int(w * (REF_SOLO_ROJA_CURVA if en_curva else REF_SOLO_ROJA_RECTA))
        cv2.circle(frame, (fx_r,  ref_inf), 6, C_LINEA_ROJA, -1)
        cv2.circle(frame, (ref_x, ref_inf), 8, (80, 80, 80), 2)
        ...
        signo = "→" if error > 0 else "←"
        curva_txt = "CURVA" if en_curva else "RECTA"
        cv2.putText(frame, f"Solo ROJA {curva_txt} err:{error:+d}px {signo}", (10, 55),
