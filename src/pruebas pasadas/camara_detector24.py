...
def detectar_y_dibujar_lineas(frame):
    ...
    if fx_r is not None and fx_a is not None:
        ...
        # Detectar curva por inclinación: diferencia X entre franja superior e inferior
        x_sup = _x_promedio_franjas(mask_r, [franjas[0]])
        x_inf = _x_promedio_franjas(mask_r, [franjas[-1]])
        if x_sup is not None and x_inf is not None:
            inclinacion = abs(x_sup - x_inf)
        else:
            inclinacion = 0
        en_curva = inclinacion > UMBRAL_INCLINACION
