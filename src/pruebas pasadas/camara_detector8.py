...
def detectar_y_dibujar_lineas(frame):
    ...
    # Solo analizar la mitad inferior del frame — menos distorsión de perspectiva
    oy  = h // 2
    roi = frame[oy:, :]
    ...
    # Detectar contornos para visualización
    ...
    # Calcular X de cada línea promediando varias franjas horizontales
    # en el tercio inferior del ROI (la zona más cercana y confiable)
    roi_h = roi.shape[0]
    franjas = [int(roi_h * p) for p in (0.60, 0.70, 0.80, 0.90)]
    ...
    # Dibujar línea de referencia en el frame completo
    ref_y = oy + int(roi_h * 0.75)
    cv2.line(frame, (0, ref_y), (w, ref_y), (50, 50, 50), 1)
    cv2.putText(frame, "ref", (4, ref_y - 4), cv2.FONT_HERSHEY_SIMPLEX, 0.35, (50, 50, 50), 1)

    error = None
    if fx_r is not None and fx_a is not None:
        cx_franja = (fx_r + fx_a) // 2
        # Marcar X de cada línea en la línea de referencia
        cv2.circle(frame, (fx_r, ref_y), 6, C_LINEA_ROJA, -1)
        cv2.circle(frame, (fx_a, ref_y), 6, C_LINEA_AMARILLA, -1)
        cv2.circle(frame, (cx_franja, ref_y), 10, C_CENTRO, -1)
        cv2.line(frame, (w//2, ref_y), (cx_franja, ref_y), C_CENTRO, 2)
        error = cx_franja - w // 2
