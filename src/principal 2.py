...
# ══════════════════════════════════════════════
#  MODO TEST (integrado en el principal)
# ══════════════════════════════════════════════
def _modo_test_local():
    """
    Modo test de motores interactivo.
...
def main():
    ...
    # ── Arrancar cámara en hilo separado ─────────────────────────────────
    # iniciar() es bloqueante (Flask), así que va en su propio hilo daemon.
    threading.Thread(
        target=camara_detector.iniciar,
        kwargs={"host": args.cam_host, "port": args.cam_port},
        daemon=True
    ).start()
    print(f"{C.INFO}[CAMARA] Streaming en http://{args.cam_host}:{args.cam_port}{C.RESET}")
    time.sleep(2)   # dar tiempo a que Flask y la cámara arranquen completamente
