...
""")
    ...
    while not stop_ev.is_set():
        ...
        if len(partes) == 2 and esp_cmd not in ("stop 0", "reset 0"):
            # Movimiento indefinido: guardar pulsos antes, enviar, no hay parada automática
            antes1, antes2 = enc["c1"], enc["c2"]
            enviar(ser, esp_cmd)
            print(f"  {C.OK}[OK]{C.RESET} {partes[0]} al {velocidad}% — indefinido")
            print(f"  {C.DIM}(encoders al arrancar — M1: {antes1:+d} pul  M2: {antes2:+d} pul){C.RESET}")

        elif len(partes) == 4 and partes[3] == "mm":
            # Movimiento por distancia — guardar pulsos antes de arrancar
            ...
            # Esperar hasta que ambas ruedas alcancen el objetivo
            while True:
                if abs(enc["c1"] - antes1) >= objetivo1 and \
                   abs(enc["c2"] - antes2) >= objetivo2:
                    enviar(ser, "stop 0")
            ...
            # Movimiento por tiempo — guardar pulsos antes de arrancar
            ms        = int(partes[2])
            direccion = partes[0]
            antes1, antes2 = enc["c1"], enc["c2"]
            enviar(ser, f"{direccion} {velocidad}")
            ...
            # Al hacer stop manual también mostrar diferencia desde el último arranque
            enviar(ser, esp_cmd)
