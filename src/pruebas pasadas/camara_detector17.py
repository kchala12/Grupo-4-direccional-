...
#  ROI_INICIO: fracción desde arriba donde empieza (0.0=arriba, 1.0=abajo)
#  ROI_ALTO:   altura de la zona como fracción del frame
# ══════════════════════════════════════════════
ROI_INICIO = 0.60
ROI_ALTO   = 0.40
...
#  DESDE/HASTA: fracción del alto del frame
#  ANCHO: fracción del ancho del frame (centrada)
#  UMBRAL_BLOQUEO: fracción del ancho que debe ocupar el obstáculo
# ══════════════════════════════════════════════
ZONA_OBSTACULO_ANCHO  = 0.6
ZONA_OBSTACULO_DESDE  = 0.40
ZONA_OBSTACULO_HASTA  = 0.75
UMBRAL_BLOQUEO        = 0.30

# ══════════════════════════════════════════════
#  CALIBRACIÓN DE PISO
#  PISO_FRAMES:   frames para calibrar
#  PISO_MARGEN_H: tolerancia de tono (hue)
#  PISO_MARGEN_S: tolerancia de saturación
#  PISO_MARGEN_V: tolerancia de brillo
#  PISO_AREA_MIN: área mínima del obstáculo en px²
# ══════════════════════════════════════════════
PISO_FRAMES   = 60
PISO_MARGEN_H = 10
PISO_MARGEN_S = 30
...
# Estado de calibración de piso
_piso_ref    = None   # (h_min, h_max, s_min, s_max, v_min, v_max)
_piso_meds   = []     # lista de medianas por frame
_piso_count  = 0
...
def resetear_fondo():
    ...
    global _piso_ref, _piso_meds, _piso_count
    with _piso_lock:
        _piso_ref   = None
        _piso_meds  = []
        _piso_count = 0
...
def _mask_lineas(hsv):
    """Devuelve máscara de píxeles que son líneas roja o amarilla."""
...
def detectar_y_dibujar_obstaculos(frame):
    ...
    # Máscara de solo piso: excluye líneas
    solo_piso_mask = cv2.bitwise_not(excl)

    with _piso_lock:
        if _piso_count < PISO_FRAMES:
            # Calibración: tomar mediana de píxeles de piso en este frame
            pixeles = hsv_roi[solo_piso_mask > 0]
            if len(pixeles) > 100:
                med_h = float(np.median(pixeles[:, 0]))
                med_s = float(np.median(pixeles[:, 1]))
                med_v = float(np.median(pixeles[:, 2]))
                _piso_meds.append((med_h, med_s, med_v))
            _piso_count += 1

            if _piso_count == PISO_FRAMES and len(_piso_meds) > 0:
                meds = np.array(_piso_meds)
                h_med = float(np.median(meds[:, 0]))
                s_med = float(np.median(meds[:, 1]))
                v_med = float(np.median(meds[:, 2]))
                _piso_ref = (
                ...
                print(f"[CAMARA] Piso: H={h_med:.0f}±{PISO_MARGEN_H} "
                      f"S={s_med:.0f}±{PISO_MARGEN_S} V={v_med:.0f}±{PISO_MARGEN_V}")
    ...
    # Piso: dentro del rango calibrado
    mask_piso = cv2.inRange(hsv_roi,
                            np.array([h_min, s_min, v_min], dtype=np.uint8),
                            np.array([h_max, s_max, v_max], dtype=np.uint8))

    # Obstáculo: ni piso ni línea
    mask_obs = np.zeros(mask_piso.shape, dtype=np.uint8)
    for y in range(mask_obs.shape[0]):
        for x in range(mask_obs.shape[1]):
            if mask_piso[y, x] == 0 and excl[y, x] == 0:
                mask_obs[y, x] = 255
