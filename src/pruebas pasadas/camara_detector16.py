...
# ══════════════════════════════════════════════
#  CALIBRACIÓN DE COLOR DE PISO
#  PISO_FRAMES:   frames para calibrar el color del piso
#  PISO_MARGEN_S: margen de saturación alrededor del promedio del piso
#  PISO_MARGEN_V: margen de valor (brillo) alrededor del promedio del piso
#  PISO_AREA_MIN: área mínima del objeto detectado en px²
# ══════════════════════════════════════════════
PISO_FRAMES   = 30
PISO_MARGEN_H = 15    # margen de hue
PISO_MARGEN_S = 40    # margen de saturación
PISO_MARGEN_V = 50    # margen de brillo
PISO_AREA_MIN = 2000

_piso_acum_h  = None
_piso_acum_s  = None
_piso_acum_v  = None
_piso_ref     = None   # (h_min, h_max, s_min, s_max, v_min, v_max)
_piso_count   = 0
_piso_lock    = threading.Lock()


def resetear_fondo():
    """Recalibra el color del piso."""
    global _piso_acum_h, _piso_acum_s, _piso_acum_v, _piso_ref, _piso_count
    with _piso_lock:
        _piso_acum_h = None
        _piso_acum_s = None
        _piso_acum_v = None
        _piso_ref    = None
        _piso_count  = 0
    print("[CAMARA] Recalibrando color de piso...")


def _mask_excl_lineas(hsv_roi):
    """Máscara de exclusión de líneas roja y amarilla."""
    m_r1 = cv2.inRange(hsv_roi, np.array([0,   60,  50]), np.array([10,  255, 255]))
    m_r2 = cv2.inRange(hsv_roi, np.array([160,  60,  50]), np.array([180, 255, 255]))
    m_a  = cv2.inRange(hsv_roi, np.array([18,   80,  80]), np.array([35,  255, 255]))
    return cv2.bitwise_or(cv2.bitwise_or(m_r1, m_r2), m_a)


# ══════════════════════════════════════════════
#  DETECCIÓN DE OBSTÁCULOS (color de piso)
# ══════════════════════════════════════════════
def detectar_y_dibujar_obstaculos(frame):
    ...
    excl     = _mask_excl_lineas(hsv_roi)
    solo_piso = cv2.bitwise_not(excl)   # solo píxeles de piso para calibrar

    with _piso_lock:
        # Fase de calibración
        if _piso_count < PISO_FRAMES:
            pixeles_piso = hsv_roi[solo_piso > 0]
            if len(pixeles_piso) > 0:
                if _piso_acum_h is None:
                    _piso_acum_h = pixeles_piso[:, 0].astype(np.float32)
                    _piso_acum_s = pixeles_piso[:, 1].astype(np.float32)
                    _piso_acum_v = pixeles_piso[:, 2].astype(np.float32)
                else:
                    _piso_acum_h = np.append(_piso_acum_h, pixeles_piso[:, 0])
                    _piso_acum_s = np.append(_piso_acum_s, pixeles_piso[:, 1])
                    _piso_acum_v = np.append(_piso_acum_v, pixeles_piso[:, 2])
            _piso_count += 1

            if _piso_count == PISO_FRAMES and _piso_acum_h is not None:
                h_med = float(np.mean(_piso_acum_h))
                s_med = float(np.mean(_piso_acum_s))
                v_med = float(np.mean(_piso_acum_v))
                _piso_ref = (
                    max(0,   h_med - PISO_MARGEN_H), min(180, h_med + PISO_MARGEN_H),
                    max(0,   s_med - PISO_MARGEN_S), min(255, s_med + PISO_MARGEN_S),
                    max(0,   v_med - PISO_MARGEN_V), min(255, v_med + PISO_MARGEN_V),
                )
                print(f"[CAMARA] Piso calibrado: H={h_med:.0f} S={s_med:.0f} V={v_med:.0f}")

            pct = int(_piso_count / PISO_FRAMES * 100)
            cv2.putText(frame, f"Calibrando piso... {pct}%",
    ...
    # Máscara de piso — lo que está dentro del rango calibrado
    h_min, h_max, s_min, s_max, v_min, v_max = ref
    mask_piso = cv2.inRange(hsv_roi,
                            np.array([h_min, s_min, v_min]),
                            np.array([h_max, s_max, v_max]))

    # Obstáculo = no es piso Y no es línea
    mask_obs = cv2.bitwise_not(cv2.bitwise_or(mask_piso, excl))

    kernel = cv2.getStructuringElement(cv2.MORPH_RECT, (7, 7))
    mask_obs = cv2.morphologyEx(mask_obs, cv2.MORPH_OPEN,  kernel)
    mask_obs = cv2.morphologyEx(mask_obs, cv2.MORPH_CLOSE, kernel)

    cnts, _ = cv2.findContours(mask_obs, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)

    bloqueando = False
    ancho_zona = zona_x2 - zona_x1
    for cnt in cnts:
        area = cv2.contourArea(cnt)
        if area < PISO_AREA_MIN:
            continue
