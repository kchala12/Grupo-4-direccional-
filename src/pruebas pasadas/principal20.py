...
# ══════════════════════════════════════════════
#  SENSOR INFRARROJO (GPIO Raspberry Pi)
#  Detecta cuando el robot está en la estación
#  IR_DETECCION_VALUE: valor del pin cuando detecta objeto
#    HIGH (1) → sensor activo en alto
#    LOW  (0) → sensor activo en bajo
# ══════════════════════════════════════════════
IR_PIN             = 17
IR_DETECCION_VALUE = 1     # cambiar a 0 si el sensor activa en LOW
IR_CICLOS_CONFIRM  = 3     # ciclos consecutivos para confirmar detección
IR_INTERVALO_S     = 0.05  # intervalo entre lecturas del IR
...
# ══════════════════════════════════════════════
#  ULTRASONIDO HC-SR04 (GPIO Raspberry Pi)
#  Detecta si la carga está presente en el robot
...
# ══════════════════════════════════════════════
#  PARÁMETROS DE CARGA
#  DELAY_POST_IR_CARGA_MS: delay desde que IR detecta hasta que para
# ══════════════════════════════════════════════
DELAY_POST_IR_CARGA_MS = 500   # ms navegando después de que IR detecta estación

# ══════════════════════════════════════════════
#  PARÁMETROS DE DESCARGA
#  DELAY_POST_IR_DESCARGA_MS: delay desde que IR detecta hasta que para
# ══════════════════════════════════════════════
DELAY_POST_IR_DESCARGA_1_MS = 500
DELAY_POST_IR_DESCARGA_2_MS = 500
DELAY_POST_IR_DESCARGA_3_MS = 500

# ══════════════════════════════════════════════
#  PARÁMETROS ULTRASONIDO
# ══════════════════════════════════════════════
DIST_CARGA_CM          = 5
DELAY_POST_CARGA_MS    = 2000
DELAY_POST_DESCARGA_MS = 2000
...
def _esperar_ir():
    """Navega hasta que el sensor IR detecta la estación. Retorna False si stop."""
    while True:
        with _autonomo_lock:
            if not _autonomo:
                return False
...
def _ejecutar_descarga(numero, tag_id):
    delay_post_ir = [None,
                     DELAY_POST_IR_DESCARGA_1_MS,
                     DELAY_POST_IR_DESCARGA_2_MS,
                     DELAY_POST_IR_DESCARGA_3_MS][numero]

    # Navegar hasta que IR detecte la estación
    print(f"{C.WARN}[DESCARGA {numero}] Esperando sensor IR...{C.RESET}")
    if not _esperar_ir():
        return

    # Delay post-IR navegando antes de parar
    print(f"{C.WARN}[DESCARGA {numero}] IR detectado — navegando {delay_post_ir}ms{C.RESET}")
    _navegar_pid(delay_post_ir / 1000.0)

    # Parar y esperar que retiren la carga
...
def _hilo_autonomo(stop_ev):
    while not stop_ev.is_set():
        ...
        # FASE 2: navegar hasta que IR detecte la estación de carga
        print(f"{C.WARN}[CARGA] Esperando sensor IR...{C.RESET}")
        if not _esperar_ir():
            continue

        # Delay post-IR navegando antes de parar
        print(f"{C.WARN}[CARGA] IR detectado — navegando {DELAY_POST_IR_CARGA_MS}ms{C.RESET}")
        _navegar_pid(DELAY_POST_IR_CARGA_MS / 1000.0)

        # Parar y esperar que pongan la carga
