...
#  Requiere N frames consecutivos para cambiar de estado
# ══════════════════════════════════════════════
OBS_FRAMES_ACTIVAR  = 3   # frames detectando para activar stop
OBS_FRAMES_LIBERAR  = 8   # frames sin detectar para liberar stop

_obs_contador = 0   # positivo = detectando, negativo = libre
...
def detectar_y_dibujar_obstaculos(frame):
    ...
    with _piso_lock:
        if _piso_count < PISO_FRAMES:
            # Acumular frames para el fondo
            if _piso_ref is None:
                _piso_ref = roi_f32.copy()
            else:
                _piso_ref = _piso_ref + roi_f32

            _piso_count += 1

            if _piso_count == PISO_FRAMES:
                _piso_ref = _piso_ref / PISO_FRAMES
        ...
        fondo = _piso_ref.copy()

    # Diferencia absoluta en los 3 canales BGR
    diff     = cv2.absdiff(roi_f32, fondo)
    # Máximo de diferencia entre los 3 canales — detecta cambio en cualquier canal
    ...
    _, thresh = cv2.threshold(diff_max, PISO_MARGEN_V, 255, cv2.THRESH_BINARY)
    kernel    = cv2.getStructuringElement(cv2.MORPH_RECT, (7, 7))
...
def _hilo_camara():
    ...
    while True:
        ...
        # Suavizado temporal del obstáculo
        global _obs_contador
        if hay_obs:
